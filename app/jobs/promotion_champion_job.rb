 class PromotionChampionJob < ApplicationJob
  ...
  def perform
    ...
    @championshipchams.each do |championshipcham|
      ...
      if ranking4.count > 8
        ranking4.sort_by { |k, v| v }.reverse.last(4).each { |k, v| puts k.update(championship_id: 61, season_id: Season.last.id - 2, number_of_points: 0) }
      else
        ranking4.sort_by { |k, v| v }.reverse.last(2).each { |k, v| puts k.update(championship_id: 61, season_id: Season.last.id - 2, number_of_points: 0) }
      end
    end
