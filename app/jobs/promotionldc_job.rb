    ...
    @championships.each do |championship|
      ...
      if championship.player_seasons.count >= 8
        ...
        @champion_bas.each do |championbas|
          if championbas.player_seasons.count <= 16
            ranking.sort_by { |k, v| v }.reverse.last(4).each { |k, v| puts k.update(championship_id: championbas.id, season_id: Season.last.id) }
          else
